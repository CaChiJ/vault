...
modified: 2025-11-08T23:35:45+09:00
cssclasses: ""
---


- 이 프로젝트는 C로 디스크 기반 B+Tree를 직접 구현하면서, 메모리 자료구조가 아니라 "페이지 단위 I/O" 관점에서 인덱스 연산을 검증하는 데 목적이 있다.
- 각 노드는 4096 byte 고정 페이지로 관리하고, 실제 읽기/쓰기는 `pread`/`pwrite`를 통해 명시적으로 수행한다.
- 즉, 이번 글의 핵심은 B+Tree의 이론 자체보다도, 디스크 I/O 비용을 고려한 Find/Insert/Delete 구현 디테일을 정리하는 것이다.
