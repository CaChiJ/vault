...
# 1. 왜 Merge Join인가?
- 우리가 구축한 디스크 기반 B+ Tree 시스템에서 두 테이블 간의 Natural Join을 수행해야 하는 요건이 주어졌다.
...
# 2. 구현
- 두 테이블을 관리하기 위한 전역 변수를 다음과 같이 선언하였다.
```c
H_P * hp;
H_P * hp2;
...
- 메인 로직인 `db_join`을 호출하면 각각의 노드들을 맨 앞단 리프 엣지로 이동시킨 다음, 각 체인을 나란히 돌면서 Sort Merge 알고리즘을 태운다. 양쪽 모두 정렬되어 있기 때문에 앞서간 인덱스를 쫓아가는 식으로 한쪽의 포인터만 전진시키면 된다.
