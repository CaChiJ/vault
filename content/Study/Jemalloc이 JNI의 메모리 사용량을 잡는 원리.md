...
- 물론 Java Heap 외에 스레드 스택이나 메타스페이스, 코드 캐시, GC 데이터 구조, Direct Buffer 등은 네이티브 메모리(Non-Heap) 공간에 잡힌다. → 이들은 `malloc`으로 할당한다.
	- 단, Spring Actuator가 보여주는 Non-Heap 공간은 위에서 언급한 JVM이 직접 할당하는 공간만 뜻한다. 그 밖에 JNI 등으로 직접 malloc 요청하는 것들은 안 잡힌다.

# 4. 진단 방법
#작성필요 - Jemalloc 프로파일링 실행 방법 - MALLOC_CONF, prof:true, jeprof
