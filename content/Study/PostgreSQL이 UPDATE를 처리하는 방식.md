...
- 이를 위해 PostgreSQL에서는 각 튜플에 `xmin`과 `xmax` 트랜잭션 ID를 함께 저장한다.
...
- 때문에 다음과 같은 방식으로 동작한다.
	...
	2. 이전 튜플이 새 튜플의 위치를 가리키는 포인터(`t_ctid`)를 저장한다 (HOT Chain 형성).
...
# 3. DB Bloat
- 그런데 PostgreSQL의 MVCC 특성상, UPDATE/DELETE 직후에 이전 버전 튜플과 인덱스 엔트리를 즉시 물리적으로 제거하지 않는다.
- 즉, 죽은 튜플(dead tuple)이 누적되면 페이지 밀도가 낮아져 파일이 비대해질 수 있다.
...
- VACUUM 이후 확보된 여유 공간 정보는 힙의 Free Space Map(FSM)에 반영되어 이후 INSERT/UPDATE에서 재사용된다.
